/**
 * ChangePassWDPage zhuyu 2018/4/18
 */
...
import style from '../../css/ChangePassWDPageStyle';
...
export default class ChangePassWDPage extends Component {
  ...
  fixPassWDEvent = () => {
    ...
    if (email.trim() === '') {
      ...
      fetch(`${this.localDataStore.url}/User/fixPassWD/`, {
        method: 'post',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        ...
        // console.log(response);
        if (response.ok) {
          alert(PAGE_WORD.fixPassWDSuccess);
        ...
        <View style = { style.mainView }>
          ...
          <View style = { style.buttonBlockView }>
            <View style = { style.registerButtonView }>
              <UserDefButton
                text = { PAGE_WORD.fixPassWD }
                buttonStyle = { style.buttonStyle }
                buttonTextStyle = { style.buttonTextStyle }
                event = { this.fixPassWDEvent }
              />
            </View>
          </View>
        </View>
